...
<div class="container py-5" >
    <div id="container  my-5" >
        <div class="row my-5 " >
            <div class="  @if(request()->get('part')==1)  col-md-10 offset-md-1 @else col-md-12  @endif    ">
                <div class="  ">
                    <div class="progress" >
                    ...
                    <center><h5 id="progress-label">0% Progress</h5></center>
                </div>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <div class="row">
        <div class="col-md-3 " id="free-questions-sidebar" @if(request()->get('part')==1) hidden @endif>
        ...
        <div class="@if(request()->get('part')==2) col-md-9 @elseif(request()->get('part')==1) col-md-10 offset-md-1 @endif ">
            <form role="form" id="regForm" action="/addQA" method="post"  enctype="multipart/form-data">
                ...
                @foreach ($freeQuestions as $parentKey=>$question)
                    ...
                    // dd($continue_q_a);
                    // dd($question);
                    if ( array_search($question->id, array_column($continue_q_a, 'question_id')) !== false) {
                        # code...
                        $defaultAnswer = $continue_q_a[array_search($question->id, array_column($continue_q_a, 'question_id')) ] ;
                        // print_r($defaultAnswer );
                    } else {
                        # code...
                        $defaultAnswer=null;
                    }
                ...
                @endif
                {{-- {{dd(count($freeQuestions))}} --}}
                {{-- {{dd($freeQuestions)}} --}}
                <div class="tab tab-hidden"> {{--start of the tab--}}
                @endif
                <div id="{{$question->id}}"

                    class="{{$parentKey == 0 ? 'question-block': 'question-hidden'}}  question_item qkey-{{$parentKey}} my-5 py-1" >
                    ...
                    <div class="row" >
                        <div class="{{$question->type == 5  || $question->type == 8 ? "col-md-8": "col-md-12"}} padding-right:0">
                        {{-- <div class="@if($question->type == 5 || $question->type == 8) col-md-8   @else col-md-12  @endif> padding-right:0"> --}}

                            <table class= style="width: 100%">
                                <tr>
                                <td style="vertical-align: top; max-width:28px">
                                    <i class="fa fa-info-circle"  style='font-size:1.5em; color:#182b62;' data-toggle="tooltip" data-placement="top" title="{{$question->q_info}}"></i>
                                </td>
                                <td style="padding-left:10px">
                                    {{ $question->name}}?
                                    <p class="text-danger d-none"  id="q-min-warning-{{ $question->id}}">{{$question->minValTextWarning}}</p>
                                </td>
                        ...
                        <div class="{{$question->type == 5 || $question->type == 8 ? "col-md-4 padding-right:0": "col-md-12 pt-4"}} padding-right:0">
                            <div class="pl-4 ml-4">
                                ...
                                <div class="btn-group colors btngrp-question-answer answerType-1 btngrp-selectAnswer2" data-toggle="buttons">
                                    ...
                                    <label
                                        onclick="showNextQuestion({{$parentKey}})"
                                        class="
                                        btn btn-primary  {{-- $key==0?'active':'' --}}
                                        @if($defaultAnswer!=null)
                                            @if($answer->id==$defaultAnswer['qanswer_id']) active focus @endif
                                        @endif
                                        "
                                        style="width: 48%; min-height: 60px;margin:.5%; border-radius:5px!important;" >
                                        <input oninput="this.className = ''" onclick="showNextQuestion({{$parentKey}})"  type="radio" name="{{$question->id}}-{{$question->type}}" value="{{$answer->id}}" autocomplete="off"  > {{$answer->userInput}}
                                    </label>
                                    @endforeach
                                </div>
                                    ...
                                    @elseIf($question->type == 2)
                                        ...
                                        <div class="form-group ml-md-3 answerType-2" onclick="showNextQuestion({{$parentKey}})">
                                        {{-- {{dd($continue_q_a )}} --}}
                                            {{-- qtype={{$question->type}}, questionId = {{$question->id}}<br> --}}

                                            {{-- If there's a default answer --}}


                                            <select

                                            class="form-control multiple-answer-select"
                                            name="{{$question->id}}-{{$question->type}}" required>

                                            @if(!isset($defaultAnswer))
                                            ...
                                            <option
                                                ...
                                                @if($defaultAnswer!=null)
                                                    @if($answer->id==$defaultAnswer['qanswer_id']) selected @endif
                                                @endif
                                    ...
                                    @elseIf($question->type == 3)
                                        ...
                                        <div class="row answer ml-md-3 pt-2 answerType-3">
                                            <div class="btn-group colors btngrp-question-answer" data-toggle="buttons">
                                    ...
                                    <div class="row answer container-answers  pt-2 answerType-4" >
                                        <div class="col" style="padding-left:0; padding-right:0">
                                            <input
                                            onchange="showNextQuestion({{$parentKey}})"
                                    ...
                                    {{-- {{dd($defaultAnswer )}} --}}
                                        ...
                                        <div class="col" style="padding-left:0; padding-right:0 answerType-5"  onclick="showNextQuestion({{$parentKey}})">

                                            {{-- {{dd($defaultAnswer['inputVal'])}} --}}
                                            <input

                                            @if($defaultAnswer!=null)
                                                value={{$defaultAnswer['inputVal']}}
                                            @else
                                                value="0"
                                            @endif

                                                class="form-control text-right answer-val dfb-answer-number"
                                                style="text-align: right;"
                                                {{-- onchange="showNextQuestion({{$parentKey}})" --}}
                                                {{-- onclick="showNextQuestion({{$parentKey}})" --}}
                                                id="input-{{$question->id}}"
                                                type="number" oninput="this.className = ''"
                                                placeholder="Enter Value"
                                                name="{{$question->id}}-{{$question->type}}"
                                                required
                                            />
                                        ...
                                        @foreach ($question->answers as $key=>$answer)
                                            <div class="col-md-6 required">
                                                <div class="form-group">
                                                    ...
                                                    {{-- <label for="email">{{$answer->userInput}}</label><br> --}}
                                                    <label >{{$answer->userInput}}</label><br>
                                                    <input

                                                        @if($defaultAnswer!=null)
                                                            value="{{$defaultAnswer['inputValArray'][$key]}}"
                                                        @else
                                                            value="0"
                                                        @endif
                                    ...
                                    <div class="whyvaluate">
                                        <div class="form-check pl-0">
                                            <label class="form-check-label" for="radio2">
                                                <input type="radio" class="form-check-input whyValuate"  name="whyValuate" value=1 checked>
                                    ...
                                    <div class="pl-4 ">
                                        <div class="row pt-4  pt-4" >
                                            <div class="col-md-6" style="padding-left:0">
                                                <input onchange=""  id="input-name"  class="form-control answer-val" value="{{isset($transaction->name)? $transaction->name:'' }}" type="text" oninput="" placeholder="Name"  name="name" required />
                                            </div>
                                            <div class="col-md-6" style="padding-left:0">
                                                <input onchange=""  id="input-email"  class="form-control answer-val" value="{{isset($transaction->email)? $transaction->email:'' }}" type="email" oninput="" placeholder="Email Address"  name="email" required />
                                            </div>
                                        </div>
                                        <div class="row     pt-2" >
                                            <div class="col-md-6" style="padding-left:0">
                                                <input onchange=""  id="input-website"   value="{{isset($transaction->name)? $transaction->website:'' }}" class="form-control answer-val" type="text" oninput="" placeholder="Website (Optional)"  name="website"  />
                                            </div>
                                        </div>
                                        ...
                                        <div class="row  pt-2" hidden >
                                            <div class="col-md-6" style="padding-left:0">
                                                @if (isset(request()->part))
                                                    @if(request()->part==2)
                                                        <input type="text" oninput="" name="transactionId" value="{{request()->transactionId}}" />
                                                        <input type="text" oninput="" name="part" value="2" />
                                                    @elseif(request()->part==1)
                                                        <input type="text" oninput="" name="transactionId" value="{{request()->transactionId}}" />
                                                        <input type="text" oninput="" name="part" value="1" />
                                                    @else
                                                        {{-- this is part 1 --}}
                                                        <input type="text" oninput="" name="part" value="{{request()->part}}"/>
                                                    @endif
                                                @endif
...
<div class="container mb-5" id="footer-continue">
    ...
    <div class="row">
        <div class=" @if(request()->get('part')==1) col-md-12   @else  col-md-offset-3 col-md-9  @endif  ">
            <center>
        ...
        <div class="modal-body ">
            ...
            <form role="form" class="px-5" action="/freequestions" method="get"  enctype="multipart/form-data">
                ...
                <div class="form-group ">
                        ...
                        <input id="continue_website" type="text" class="form-control  value="{{isset($transaction->website)? $transaction->website:'' }}" " placeholder="Website">

                </div>
                <div class="form-group ">

                        <input id="continue_name" type="text" class="form-control"  placeholder="Name" value="{{isset($transaction->name)? $transaction->name:'' }}" required>
                ...
                <div class="form-group ">
                        ...
                        <input id="continue_email" type="email" class="form-control"  placeholder="Email"   value="{{isset($transaction->email)? $transaction->email:'' }}" required>
...
<script>
	...
	echo " var questionCount = ". count($freeQuestions) . ";\n";
    ...
    var traffic_source =  settings.filter( function(settings){return (settings.name=='traffic_source');} );


    var input_revenue_12_months_value = document.getElementById('input-'+total_revenue_12_months_id[0]['value']);
    var input_cost_goods_sold_12_months_value =  document.getElementById('input-'+total_cost_goods_sold_12_months_id[0]['value']);
    var input_operating_expense_12_months_value  = document.getElementById('input-'+total_operating_expense_12_months_id[0]['value']);
    // var input_operating_expense_12_months_value  = document.getElementById(traffic_source);
    // var input_operating_expense_12_months_value  = document.querySelector('#'+ traffic_source);

    // console.log(input_revenue_12_months_value );
    input_revenue_12_months_value.min = 1000;

    // document.getElementById(input_revenue_12_months_value).value=999999999;

    // console.log(total_revenue_12_months_id[0]['value']  );
    // console.log(total_revenue_12_months_id[0].value  );

    // var progressCount =10; //set this on page load in a hidden field after an ajax call
    // var total = document.getElementById('total').innerHTML; //set this on initial page load
    var progressTotal = 100; //set this on initial page load
    // var pgressPcg = Math.floor(progressCount/ progressTotal*100);

    // var progressQuestionPercenter = 100/ document.getElementsByClassName("tab").length;
    var progressQuestionPercenter =100/5    ;
    // document.getElementsByClassName('progress-bar').item(0).setAttribute('aria-valuenow', parseInt(progressQuestionPercenter) );
    // document.getElementsByClassName('progress-bar').item(0).setAttribute('style','width:'+ parseInt(progressQuestionPercenter) );
    // document.getElementById('progress-label').innerHTML = parseInt(progressQuestionPercenter) + '%' + 'Progress';


    // alert(progressQuestionPercenter );
...
function findTotalTrafficSource(){
  ...
  function showNextQuestion(n){
    ...
    // var progressCount = Number(document.getElementById('count').innerHTML); //set this on page load in a hidden field after an ajax call
    // // var total = document.getElementById('total').innerHTML; //set this on initial page load
    // var progressTotal = 10; //set this on initial page load
    // var pgressPcg = Math.floor(count/total*100);

    // document.getElementsByClassName('progress-bar').item(0).setAttribute('aria-valuenow',pgressPcg );
  ...
  function nextPrev(n) {
    ...
    var progressVal = document.getElementsByClassName('progress-bar').item(0).getAttribute('aria-valuenow')
    // var progressStyleVal =  document.getElementsByClassName('progress-bar').item(0).style.width;
    // var progressVal =0;
    var progressStyleVal =  document.getElementsByClassName('progress-bar').item(0).style.width;
    ...
    document.getElementsByClassName('progress-bar').item(0).setAttribute('aria-valuenow', parseInt(progressVal) +  parseInt(progressQuestionPercenter ));
    document.getElementsByClassName('progress-bar').item(0).setAttribute('style',' width:'+  (parseInt(progressVal) +  parseInt(progressQuestionPercenter ) +'%' )    );
    document.getElementById('progress-label').innerHTML =  parseInt(progressVal) +  parseInt(progressQuestionPercenter ) + '%' + 'Progress';
  ...
  function validateForm() {
	...
	if(currentTab==0){
        ...
        var total_revenue_12_months_value =   input_revenue_12_months_value.value;
        var total_cost_goods_sold_12_months_value =   input_cost_goods_sold_12_months_value.value;
        var total_operating_expense_12_months_value =   input_operating_expense_12_months_value.value;
        ...
        if (netprofit<0){
            ...
            document.getElementById('q-min-warning-5').classList.remove("d-none");
            ...
            document.getElementById('q-min-warning-5').classList.add("d-none");
        }
  ...
  function fixStepIndicator(n) {
    ...
    // for sidebar
        // This function removes the "active" class of all steps...
        // n sidebar-finished   sidebar-current


        var i, x = document.getElementsByClassName("sidebar-btn");


        for (i = 0; i < x.length; i++) {


            // x[i].className = x[i].className.replace("sidebar-current", "");
            console.log(i + '-' + n)
            if(i==n){
                x[n].className += "sidebar-current";
            }
            else if(i<n){
                x[i].className = x[i].className.replace("sidebar-current", "sidebar-finished");

                //  x[i].className.remove("sidebar-finished")

            }
            else if(i>n){
                // x[i].className.remove("sidebar-current");
                //  x[i].className.remove("sidebar-finished")
